...
#define Q ahrs->quaternion.element
    ...
    if (FusionVectorIsZero(magnetometer) == false) {
        ...
        const FusionVector halfWest = {
                .axis{.x=Q.x * Q.y + Q.w * Q.z, .y=Q.w * Q.w - 0.5f + Q.y * Q.y, .z=Q.y * Q.z - Q.w * Q.x}
 ...
 * @param ahrs AHRS algorithm structure.
 * @param gyroscope Gyroscope measurement in degrees per second.
 * @param accelerometer Accelerometer measurement in g. * @param heading Heading measurement in degrees.
 * @param deltaTime Delta time in seconds.
 */
...
#define Q ahrs->quaternion.element
    ...
    const FusionVector magnetometer = {
            .axis{.x=cosf(headingRadians), .y=-1.0f * cosf(roll) * sinHeadingRadians, .z=sinHeadingRadians * sinf(roll)}
...
#define Q ahrs->quaternion.element
    const FusionVector gravity = {
            .axis{.x=2.0f * (Q.x * Q.z - Q.w * Q.y), .y=2.0f * (Q.y * Q.z + Q.w * Q.x), .z=(Q.w * Q.w - 0.5f + Q.z * Q.z)}
...
#define A ahrs->accelerometer.axis
    ...
    const FusionVector earthAcceleration = {
            .axis{.x=2.0f * ((qwqw - 0.5f + Q.x * Q.x) * A.x + (qxqy - qwqz) * A.y + (qxqz + qwqy) * A.z), .y=2.0f * ((qxqy + qwqz) * A.x + (qwqw - 0.5f + Q.y * Q.y) * A.y + (qyqz - qwqx) * A.z), .z= (2.0f * ((qxqz - qwqy) * A.x + (qyqz + qwqx) * A.y + (qwqw - 0.5f + Q.z * Q.z) * A.z)) - 1.0f}
